...
export default function HomePage() {
  ...
  function clickPage(e, x) {
    homeImg.map((val, i) => {
      ul.current.children[i].children[0].style.color = "rgb(82,82,91)";
      ul.current.children[i].children[0].children[2].style.backgroundColor =
        "white";
    });
    setNumberImg(x);
    ul.current.children[x].children[0].style.color = "rgb(0,119,219)";
    ul.current.children[x].children[0].children[2].style.backgroundColor =
      "rgb(0,119,219)";
  }

  const p1 = useRef();
  function listItem11(e, x) {
    if (x == 0) {
      p1.current.innerHTML = "یک طرفه";
    } else if (x == 1) {
      p1.current.innerHTML = "رفت و برگشت";
    }
  }
  ...
  const [searchItem11, setSearchItem11] = useState("");
  const [searchItem12, setSearchItem12] = useState("");
  const [searchResults, setSearchResults] = useState([]);
  const ulCity11 = useRef();
  const ulCity12 = useRef();
  function handelInputChange11(e) {
      ulCity11.current.style.display = "flex";
      ulCity12.current.style.display = "none";
    setSearchItem11(e.target.value);
    const results = airport.filter((city) => city.includes(e.target.value));
    setSearchResults(results);
  }
  function item11(e,x){
    ulCity11.current.style.display = "none";
    setSearchItem11(e.target.innerHTML)
  }
  function handelInputChange12(e) {
      ulCity12.current.style.display = "flex";
      ulCity11.current.style.display = "none";
      setSearchItem12(e.target.value);
      const results = airport.filter((city) => city.includes(e.target.value));
      setSearchResults(results);
    }
    function item12(e,x){
      ulCity12.current.style.display = "none";
      setSearchItem12(e.target.innerHTML)
    }

  return (
    <section className="w-[100%] h-[45vh] relative">
      <article className="w-[100%] h-[100%] absolute top-0">
        <Image
          src={homeImg[numberImg]}
      ...
      <article className="w-[100%] h-[36vh] absolute bottom-[-24vh] flex flex-wrap justify-center items-center">
        <ul
          ref={ul}
          className="w-[79%] h-[12vh] bg-white rounded-t-md border-zinc-300 border flex justify-center"
        >
          <li className="w-[12%] h-[100%] flex">
            <button
              className="w-[100%] h-[100%] text-[1.1rem] text-zinc-600 flex flex-wrap justify-center content-end"
              onClick={() => clickPage(event, 0)}
            >
              ...
              <p className="w-[100%] h-[40%] flex justify-center items-center">
                پرواز داخلی
              </p>
              <span className="w-[80%] h-[5%]  rounded-t-2xl"></span>
            </button>
          </li>
          <li className="w-[12%] h-[100%] flex">
            <button
              className="w-[100%] h-[100%] text-[1.1rem] text-zinc-600 flex flex-wrap justify-center content-end"
              onClick={() => clickPage(event, 1)}
            >
              <div className="w-[100%] h-[40%] flex justify-center items-center">
                <svg
                  viewBox="0 0 24 24"
        ...
        <div className="w-[79%] h-[24vh] border rounded-b-lg">
          <div
            style={{ display: "flex" }}
            className="w-[100%] h-[100%] flex flex-wrap justify-center content-center px-8"
          >
            <ul className="w-[100%] h-[40%] border  flex justify-start items-center">
              <li className="group h-[2rem] border border-zinc-400 text-[1rem] relative text-zinc-600 cursor-pointer rounded-2xl flex justify-center gap-1 items-center  px-2">
                <p ref={p1}>یک طرفه</p>
                <i className="bi bi-chevron-down"></i>

                <ul className="group-hover:flex bg-white hidden w-[130%] absolute top-[100%] right-0 rounded-md border justify-center content-center flex-wrap shadow-lg">
                  <li className="w-[100%] h-[5vh]  p-1">
                    <button
                      onClick={() => listItem11(event, 0)}
                      className="w-[100%] h-[100%] flex justify-start items-center text-[.8rem] text-zinc-500 hover:bg-[rgb(242,249,255)] rounded-md px-1 transition-all duration-300"
                    >
                      یک طرفه
                    </button>
                  </li>
            ...
            <div className="w-[100%] h-[60%]  flex">
              <ul className="w-[40%] h-[100%] flex justify-center items-center">
                <li className="w-[48%] h-[3.5rem] relative">
                  <fieldset className="w-[100%] h-[100%] border-2 overflow-hidden rounded-r-2xl">
                    <legend className="text-[.7rem] ms-2">مبدا(شهر)</legend>
                    <input
                      type="text"
                      value={searchItem11}
                      onChange={() => handelInputChange11(event)}
                      className="w-[100%] h-[100%] px-8 focusBorder"
                    />
                  </fieldset>
                  <ul
                    ref={ulCity11}
                    style={{ display: "none" }}
                    className="w-[100%] h-[30vh] h-min-[5vh] flex flex-wrap overflow-y-scroll scroll border absolute top-[100%] bg-white"
                  >
                    {searchResults &&
                      searchResults.map((val, i) => {
                        return <li className="w-[100%] h-[5vh] cursor-pointer px-2 hover:bg-[rgb(242,249,255)]" onClick={()=>item11(event,i)}>{val}</li>;
                      })}
                  </ul>
                </li>

                <li className="w-[48%] h-[3.5rem] relative">
                  <fieldset className="w-[100%] h-[100%] border-2 overflow-hidden rounded-l-2xl">
                    <legend className="text-[.7rem] ms-2">مقصد(شهر)</legend>
                    <input
                      type="text"
                      value={searchItem12}
                      onChange={() => handelInputChange12(event)}
                      className="w-[100%] h-[100%] px-8 focusBorder"
                    />
                  </fieldset>
                  <ul
                  ...
                  >
                    {searchResults &&
                      searchResults.map((val, i) => {
                        return <li className="w-[100%] h-[5vh] cursor-pointer px-2 hover:bg-[rgb(242,249,255)]" onClick={()=>item12(event,i)}>{val}</li>;
                      })}
                  </ul>
                </li>
              </ul>
          ...
          <div
            style={{ display: "none" }}
            className="w-[100%] h-[100%] flex flex-wrap justify-center content-center"
          ></div>
